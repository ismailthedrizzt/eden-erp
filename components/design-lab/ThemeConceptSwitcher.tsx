    ...
    <section className="rounded-[var(--dl-card-radius)] border border-[var(--dl-border-subtle)] bg-[var(--dl-surface-raised)] p-4" style={{ boxShadow: 'var(--dl-shadow-subtle)' }}>
      <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--dl-text-secondary)]">Ayni ERP bloklarini bes farkli gorsel kimlik tokeniyle karsilastir.</p>
        </div>
        <span className="text-xs font-semibold text-[var(--dl-text-muted)]">Active: {concepts.find(item => item.id === activeId)?.name} / {appearance}</span>
      </div>
