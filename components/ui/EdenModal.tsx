  ...
  return (
    <Modal open={open} onClose={onClose} title={title} footer={footer} size="2xl">
      ...
      <div className="-mx-6 -my-5 flex flex-col min-h-[60vh] max-h-[75vh]">
        ...
        {/* Tab Menüsü */}
        <div className="flex flex-wrap gap-2 border-b border-gray-200 bg-white px-2 py-2 dark:border-gray-700 dark:bg-eden-navy-2 shrink-0">
          {tabs.map((tab, idx) => (
            <button
              key={idx}
              onClick={() => setActiveTab(idx)}
              className={cn(
                'w-40 max-w-full rounded-lg border px-4 py-3 text-left text-sm font-medium transition-all flex items-center gap-2',
                activeTab === idx
                  ? 'border-blue-200 bg-blue-50 text-blue-600 dark:border-blue-900/60 dark:bg-blue-950/30 dark:text-blue-400'
                  : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300 hover:text-gray-700 dark:border-gray-800 dark:bg-eden-navy-2 dark:text-gray-400 dark:hover:border-gray-700 dark:hover:text-gray-200'
              )}
            >
              {tab.icon}
              {tab.label}
