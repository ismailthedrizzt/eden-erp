...
const SOURCE_URL = 'https://tr.ibanvalidator.com/support/bank/13/turkiye-is-bankasi-as.html'
const OUTPUT_PATH = path.join(__dirname, '..', 'lib', 'data', 'isbank-branches.json')
...
async function main() {
  ...
  const payload = {
    sources: [
      {
        key: 'ibanValidatorIsbankBranches',
        name: 'IBAN Validator - Türkiye İş Bankası A.Ş. şube listesi',
        url: SOURCE_URL,
      },
    ],
  ...
  console.log(`Wrote ${Object.keys(branches).length} İş Bankası branch codes to ${OUTPUT_PATH}`)
}
