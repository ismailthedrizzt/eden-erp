...
assertIncludes('components/ui/formControlStyles.ts', 'export function formControlClass', 'form controls must use the shared visual style helper')
assertIncludes('components/ui/EntityForm.tsx', 'className: baseInputClass', 'EntityForm custom renderers must receive template control classes')
assertIncludes('components/ui/EntityForm.tsx', 'validationState,', 'EntityForm custom renderers must receive centralized validation state')
assertIncludes('app/globals.css', '-webkit-text-fill-color', 'disabled/read-only controls must keep readable text fill colors')
...
assertIncludes('docs/UI_UX_DESIGN_RULES.md', 'formControlClass', 'UX rules must document the shared form control visual contract')
assertIncludes('docs/templates/ERPPageTemplate-UPDATED.md', 'formControlClass', 'ERP page template must guard the shared form control visual contract')
assertIncludes('FrontendDataAccessRules.md', 'Form gorsel kontrati', 'frontend rules must document the shared form control visual contract')
...
assertIncludes('docs/templates/FastEntityListTemplate.md', 'resizeImageFileAsDataUrl', 'fast list template must require avatar/image downsizing before persistence')
...
assertIncludes('components/ui/EntityForm.tsx', 'async function createImageVariantsOnServer', 'EntityForm image uploads must downsize avatars before persistence')
assertIncludes('components/ui/EntityForm.tsx', 'avatarImageMaxDimension(image.slotId)', 'EntityForm dataField image uploads must use avatar-aware dimensions')
...
assertIncludes('app/api/ik/personel/route.ts', "missingColumn === 'record_status'", 'employee list GET must fall back when legacy databases do not have record_status yet')
assertIncludes('app/api/ik/personel/route.ts', 'canFilterRecordStatus = false', 'employee list GET must stop filtering passive rows when record_status is unavailable')
