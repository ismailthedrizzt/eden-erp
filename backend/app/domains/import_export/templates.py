...
TEMPLATES: dict[str, ImportTemplate] = {
    ...
    "bank_transactions_template": _template(
        template_key="bank_transactions_template",
        module_key="accounting",
        entity_type="bank_transaction",
        label="Banka Hareketleri",
        description="CSV/XLSX banka ekstresi aktarimi; cari hareket dogrudan olusturmaz, mutabakat onerisi uretir.",
        rules=[
            _rule("company_id", "Sirket ID", required=True),
            _rule("bank_account_id", "Banka hesabi ID", required=True),
            _rule("transaction_date", "Islem tarihi", required=True, data_type="date"),
            _rule("value_date", "Valor tarihi", data_type="date"),
            _rule("description", "Aciklama", required=True),
            _rule("counterparty_name", "Karsi taraf"),
            _rule("counterparty_iban", "Karsi taraf IBAN"),
            _rule("amount", "Tutar", required=True, data_type="decimal"),
            _rule("direction", "Borc/Alacak", required=True, enum_values=["debit", "credit"]),
            _rule("currency", "Para birimi", data_type="currency"),
            _rule("balance_after", "Hareket sonrasi bakiye", data_type="decimal"),
            _rule("bank_reference_no", "Banka referans no"),
            _rule("transaction_code", "Islem kodu"),
            _rule("notes", "Notlar"),
            _rule("reconciliation_status", "Mutabakat durumu", controlled=True),
        ],
        sample_rows=[
            {
                "company_id": "00000000-0000-0000-0000-000000000000",
                ...
                "direction": "credit",
                "currency": "TRY",
                "bank_reference_no": "BNK-2026-0001",
            }
        ],
        hints={"bank_reference_no": ["Referans", "Dekont No"], "counterparty_name": ["Gonderen", "Alici", "Karsi Taraf"]},
        operation_controlled_fields=["reconciliation_status"],
    ),
    "card_transactions_template": _template(
        template_key="card_transactions_template",
        module_key="accounting",
        entity_type="card_transaction",
        label="Kredi Karti Hareketleri",
        description="Sirket kredi karti ekstresi aktarimi; belge yoksa Action Center uyarisi uretilebilir.",
        rules=[
            _rule("company_id", "Sirket ID", required=True),
            _rule("card_account_id", "Kart hesabi ID", required=True),
            _rule("transaction_date", "Islem tarihi", required=True, data_type="date"),
            _rule("posting_date", "Ekstre tarihi", data_type="date"),
            _rule("merchant_name", "Isyeri"),
            _rule("description", "Aciklama", required=True),
            _rule("amount", "Tutar", required=True, data_type="decimal"),
            _rule("currency", "Para birimi", data_type="currency"),
            _rule("installment_count", "Taksit sayisi", data_type="integer"),
            _rule("installment_no", "Taksit no", data_type="integer"),
            _rule("category", "Kategori"),
            _rule("document_status", "Belge durumu", enum_values=["no_document", "document_needed", "document_uploaded"]),
            _rule("reconciliation_status", "Mutabakat durumu", controlled=True),
        ],
        sample_rows=[{"company_id": "00000000-0000-0000-0000-000000000000", "card_account_id": "00000000-0000-0000-0000-000000000002", "transaction_date": "2026-05-29", "merchant_name": "Kirtasiye", "description": "Ofis sarf malzemesi", "amount": "1250.00", "currency": "TRY"}],
        operation_controlled_fields=["reconciliation_status"],
    ),
    "e_documents_template": _template(
        template_key="e_documents_template",
        module_key="accounting",
        entity_type="e_document",
        label="e-Fatura / e-Arsiv",
        description="e-Fatura/e-Arsiv metadata aktarimi; XML/PDF dosyalari Document domain iliskisiyle baglanir.",
        rules=[
            _rule("company_id", "Sirket ID", required=True),
            _rule("document_kind", "Belge turu", required=True, enum_values=["e_invoice", "e_archive", "paper_invoice", "receipt", "other"]),
            _rule("direction", "Yon", required=True, enum_values=["incoming", "outgoing"]),
            _rule("invoice_uuid", "Fatura UUID"),
            _rule("invoice_no", "Fatura no", required=True),
            _rule("issue_date", "Duzenleme tarihi", required=True, data_type="date"),
            _rule("due_date", "Vade tarihi", data_type="date"),
            _rule("sender_tax_number", "Gonderen VKN"),
            _rule("sender_name", "Gonderen unvan"),
            _rule("receiver_tax_number", "Alici VKN"),
            _rule("receiver_name", "Alici unvan"),
            _rule("total_amount", "Toplam", required=True, data_type="decimal"),
            _rule("tax_amount", "Vergi", data_type="decimal"),
            _rule("payable_amount", "Odenecek tutar", data_type="decimal"),
            _rule("currency", "Para birimi", data_type="currency"),
            _rule("status", "Belge durumu", enum_values=["received", "issued", "accepted", "rejected", "cancelled", "needs_review"]),
            _rule("related_cari_account_id", "Bagli cari kart"),
            _rule("xml_document_id", "XML document ID"),
            _rule("pdf_document_id", "PDF document ID"),
            _rule("reconciliation_status", "Mutabakat durumu", controlled=True),
        ],
        sample_rows=[{"company_id": "00000000-0000-0000-0000-000000000000", "document_kind": "e_archive", "direction": "incoming", "invoice_no": "EAR202600001", "issue_date": "2026-05-29", "sender_tax_number": "1234567890", "sender_name": "ABC Tedarik A.S.", "total_amount": "5000.00", "tax_amount": "900.00", "payable_amount": "5900.00", "currency": "TRY"}],
        hints={"invoice_no": ["Fatura No", "Belge No"], "invoice_uuid": ["UUID", "ETTN"]},
        operation_controlled_fields=["reconciliation_status"],
    ),
