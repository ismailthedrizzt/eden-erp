...
interface StakeholderRow {
  id: string
  company_id?: string
  stakeholder_type?: 'person' | 'organization'
  category?: string
  display_name?: string
  tax_id?: string
  phone?: string
...
const tabs: FormTab[] = [
  {
    ...
    label: 'Genel',
    fields: [
      { name: 'birth_place', label: 'Doğum Yeri', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      {
        name: 'gender',
        label: 'Cinsiyet',
        type: 'select',
        visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' },
        options: [
          { value: 'male', label: 'Erkek' },
          { value: 'female', label: 'Kadın' },
        ],
      },
      { name: 'birth_date', label: 'Doğum Tarihi', type: 'date', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      { name: 'occupation', label: 'Meslek', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      { name: 'marital_status', label: 'Medeni Durum', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      { name: 'expertise_area', label: 'Uzmanlık Alanı', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      { name: 'employer_company', label: 'Çalıştığı Firma', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      { name: 'linkedin', label: 'LinkedIn', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'person' } },
      { name: 'foundation_date', label: 'Kuruluş Tarihi', type: 'date', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' } },
      { name: 'company_type', label: 'Şirket Türü', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' } },
      { name: 'tax_office', label: 'Vergi Dairesi', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' } },
      { name: 'mersis_number', label: 'MERSİS', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' } },
      { name: 'trade_registry_no', label: 'Ticaret Sicil No', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' } },
      { name: 'website', label: 'Web Sitesi', type: 'text', visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' } },
  ...
  { id: 'notes', label: 'Notlar', fields: [{ name: 'notes', label: 'Notlar', type: 'textarea', colSpan: 3 }] },
  {
    id: 'gecmis',
    label: 'Geçmiş',
    fields: [
...
export default function PaydaslarPage() {
  ...
  const tableData = useMemo(() => stakeholders.map(stakeholder => ({
    ...stakeholder,
    stakeholder_type_label: stakeholder.stakeholder_type === 'organization' ? 'Tüzel Kişi' : 'Gerçek Kişi',
  ...
  const widgets: WidgetDef<any>[] = useMemo(() => [
    ...
    { key: 'legal', label: 'Tüzel Kişi', render: () => tableData.filter(row => row.stakeholder_type === 'organization').length },
  ...
  const configuredTabs = [
    ...
    ...createLegalEntityMasterTabs({
      visibleWhen: { field: 'stakeholder_type', operator: 'equals', value: 'organization' },
    ...
    <div className="relative">
      ...
      {pageState === 'list' && (
        <div className="mt-6">
          ...
          <SmartDataTable columns={columns} data={tableData} loading={loading} widgets={widgets} defaultView="list" storageKey="companies-stakeholders-table" emptyText="Paydaş kaydı bulunamadı" onRowClick={handleRowClick} onRefresh={() => loadData(true)} defaultPageSize={listQuery.pageSize} pagination={{ mode: 'server', page: listMeta.page, pageSize: listMeta.pageSize, total: listMeta.total, onPageChange: page => setListQuery(prev => ({ ...prev, page })), onPageSizeChange: pageSize => setListQuery(prev => ({ ...prev, page: 1, pageSize })), onSearchChange: search => setListQuery(prev => ({ ...prev, page: 1, search })), onSortChange: handleListSortChange }} showPassiveToggle includePassive={includePassive} onIncludePassiveChange={(next) => { setIncludePassive(next); setListQuery(prev => ({ ...prev, page: 1 })) }} />
        </div>
      )}
        ...
        <div className="mt-6">
          <EntityForm
            ...
            imageSlot={{
              ...
              slots: (formData) => normalizeStakeholderEntityType(formData.stakeholder_type || formData.master_entity_kind) === 'organization'
                ? [
...
function normalizePayload(raw: Record<string, any>, companies: Option[], current?: Record<string, any>) {
  ...
  if (payload.master_entity_kind === 'person') payload.stakeholder_type = 'person'
  if (payload.master_entity_kind === 'organization') payload.stakeholder_type = 'organization'
  payload.stakeholder_type = payload.stakeholder_type || effective.stakeholder_type || 'person'
  payload.display_name = payload.stakeholder_type === 'organization'
    ? effective.trade_name || effective.legal_name || effective.trade_name || effective.short_name || effective.short_name || effective.display_name
  ...
  payload.tax_id = payload.tax_id || payload.national_id || payload.national_id || payload.tax_number || payload.tax_number || payload.passport_no || payload.passport_no
