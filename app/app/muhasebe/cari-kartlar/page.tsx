...
const columns: ColumnDef[] = [
  ...
  { key: 'entity_label', label: 'Kiþi / Kurum', type: 'text', width: 130, category: 'Kimlik' },
  { key: 'roles_label', label: 'Roller', type: 'text', width: 180, category: 'Ýliþki' },
...
const financeFields: FormField[] = [
  ...
  { name: 'default_currency', label: 'Varsayýlan Para Birimi', type: 'select', options: [{ value: 'TRY', label: 'TRY' }, { value: 'USD', label: 'USD' }, { value: 'EUR', label: 'EUR' }] },
...
const detailTabs: FormTab[] = [
  { id: 'hareketler', label: 'Hareketler', fields: [{ name: 'movement_summary', label: 'Hareket Özeti', type: 'custom', colSpan: 3, render: ({ value }) => <ReadOnlyBox text={value || 'Hareketler Ön Muhasebe Hareketleri ekranýnda yönetilir.'} /> }] },
  { id: 'banka_bilgileri', label: 'Banka Bilgileri', fields: [{ name: 'entity_bank_accounts', label: 'Banka Bilgileri', hideLabel: true, type: 'custom', colSpan: 3, render: ({ data, readOnly }) => <EntityBankAccountsPanel entityKind={data.entity_kind} entityId={data.person_id || data.organization_id} masterName={data.display_name} masterCountry={data.country || data.nationality || data.country} readOnly={readOnly} /> }] },
  { id: 'finans', label: 'Finans Ayarlarý', fields: financeFields },
  { id: 'mutabakat', label: 'Mutabakat', fields: [{ name: 'reconciliation_summary', label: 'Mutabakat', type: 'custom', colSpan: 3, render: () => <ReadOnlyBox text="Mutabakat akýþý için hazýr alan." /> }] },
  { id: 'baglantilar', label: 'Baðlantýlar', fields: [{ name: 'links', label: 'Baðlý Kayýt', type: 'custom', colSpan: 3, render: () => <ReadOnlyBox text="Kimlik deðiþiklikleri baðlý kaynak modülden yapýlmalýdýr." /> }] },
  { id: 'belgeler', label: 'Belgeler', fields: [{ name: 'documents', label: 'Belgeler', type: 'custom', colSpan: 3, render: () => <ReadOnlyBox text="Belge baðlantýlarý hareketler üzerinden eklenecek." /> }] },
  { id: 'gecmis', label: 'Geçmiþ', fields: [{ name: 'history', label: 'Geçmiþ', type: 'custom', colSpan: 3, render: () => <ReadOnlyBox text="Finans ayarý ve hareket geçmiþi audit log üzerinden izlenecek." /> }] },
]
...
export default function AccountCardsPage() {
  ...
  const tableData = useMemo(() => cards.map(card => ({
    ...
    entity_label: card.entity_kind === 'person' ? 'Gerçek Kiþi' : 'Tüzel Kiþi',
    roles_label: card.roles?.length ? card.roles.join(', ') : '-',
    identity_display: card.identity_no || card.tax_no || '-',
    risk_label: card.risk_status === 'limit_exceeded' ? 'Limit Aþýldý' : 'Normal',
  ...
  const widgets: WidgetDef<any>[] = useMemo(() => [
    ...
    { key: 'person', label: 'Gerçek Kiþi', render: () => tableData.filter(row => row.entity_kind === 'person').length },
    { key: 'organization', label: 'Tüzel Kiþi', render: () => tableData.filter(row => row.entity_kind === 'organization').length },
  ...
  const heroFields: FormField[] = [
    ...
    { name: 'entity_label', label: 'Kiþi / Kurum Tipi', type: 'custom', render: ({ value }) => <ReadOnlyField value={value} /> },
  ...
  const saveSettings = async (data: Record<string, any>) => {
    ...
    try {
      ...
      setToast({ type: 'success', title: 'Kaydedildi', message: 'Cari finans ayarlarý güncellendi.' })
      setMode('view')
      await loadCards()
    } catch (error) {
  ...
  return (
    <div className="relative">
      <PageBanner
        ...
        subtitle={selectedCard ? 'Cari kart görünümü ve finans ayarlarý' : 'Kiþi ve kurumlarýn finansal iliþkilerini, bakiyelerini ve hareketlerini görüntüleyin.'}
        icon={<WalletCards size={24} />}
        onAddClick={!selectedCard && can(ACCOUNTING_PERMISSIONS.accountCardsEditFinancialSettings) ? () => setLauncherOpen(true) : undefined}
        addButtonText="Yeni Cari Ýliþkisi Ekle"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-2xl rounded-xl bg-white p-5 shadow-xl dark:bg-gray-900">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Yeni Cari Ýliþkisi Ekle</h3>
            <p className="mt-1 text-sm text-gray-500">Cari kart kimlik oluþturmaz; doðru kaynak formu açar.</p>
          </div>
          <button onClick={onClose} className="btn">Kapat</button>
        </div>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label className="space-y-1 text-sm">
            <span>Kiþi / Kurum Tipi</span>
            <select value={entityKind} onChange={(e) => { setEntityKind(e.target.value as AccountingEntityKind); setResult(null) }} className="w-full rounded-lg border px-3 py-2 dark:bg-gray-800">
              <option value="person">Gerçek Kiþi</option>
              <option value="organization">Tüzel Kiþi</option>
            </select>
          </label>
          {entityKind === 'person' ? (
            <>
              <Input label="Uyruðu" value={identity.nationality || ''} onChange={v => setIdentity(p => ({ ...p, nationality: v }))} />
        ...
        <button onClick={search} disabled={loading} className="btn btn-primary mt-4">{loading ? 'Aranýyor...' : 'Master Kaydý Ara'}</button>
            ...
            {result.found ? (
              <div className="mt-3 space-y-2 text-sm">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  ...
                  <button className="btn">Finans Ayarlarýný Düzenle</button>
                  <button className="btn">Yeni Rol / Ýliþki Ekle</button>
                </div>
              </div>
            ) : (
              <div className="mt-3">
                <p className="text-sm text-amber-700 dark:text-amber-300">Önce ilgili iþ formundan master kayýt oluþturulmalýdýr.</p>
