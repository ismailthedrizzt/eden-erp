import { Headphones, PackageCheck, Tags } from 'lucide-react'
import Link from 'next/link'
import { productServicesHubPageContract } from '@/contracts/pages/product-services/product-services-hub.page.contract'
import { PageBanner } from '@/components/ui/PageBanner'

const iconByKey = { Headphones, PackageCheck, Tags } as const

type IconKey = keyof typeof iconByKey

function iconFor(key: string, className = 'h-5 w-5') {
  const Icon = iconByKey[key as IconKey] ?? Tags
  return <Icon className={className} />
}

export default function ProductServicesPage() {
  const { dashboard } = productServicesHubPageContract
