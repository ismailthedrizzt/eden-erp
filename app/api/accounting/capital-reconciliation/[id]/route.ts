...
// TARGET_FASTAPI_ENDPOINT: /api/v1/accounting/capital-reconciliation/{id}
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await proxyToFastApi(request, `/api/v1/accounting/capital-reconciliation/${id}`)
