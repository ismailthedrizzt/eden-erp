...
import { BANK_ACCOUNT_SELECT, BANK_CONNECTION_SELECT } from '../../../bank-accounts-cards/_shared'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('bank_accounts').select(BANK_ACCOUNT_SELECT).eq('bank_connection_id', id).eq('is_deleted', false).order('created_at', { ascending: false })
  if (error) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: connection } = await supabase.from('bank_connections').select(BANK_CONNECTION_SELECT).eq('id', id).maybeSingle()
  ...
  const { data, error } = await supabase.from('bank_accounts').insert({ ...payload, created_by: permission.userId, updated_by: permission.userId }).select(BANK_ACCOUNT_SELECT).single()
