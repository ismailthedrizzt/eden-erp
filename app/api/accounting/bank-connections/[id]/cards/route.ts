...
import { BANK_CARD_SELECT, BANK_CONNECTION_SELECT } from '../../../bank-accounts-cards/_shared'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('bank_cards').select(BANK_CARD_SELECT).eq('bank_connection_id', id).eq('is_deleted', false).order('created_at', { ascending: false })
  if (error) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('bank_cards').insert({ ...payload, created_by: permission.userId, updated_by: permission.userId }).select(BANK_CARD_SELECT).single()
