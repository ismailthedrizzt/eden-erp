...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ cardId: string }> }) {
  ...
  const { data, error } = await supabase.from('bank_cards').update({ ...payload, updated_at: new Date().toISOString(), updated_by: permission.userId }).eq('id', cardId).select('*').single()
