...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase.from('financial_institution_movements').insert({ ...payload, created_by: permission.userId, updated_by: permission.userId }).select('*').single()
