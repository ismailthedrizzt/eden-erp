...
export async function POST(request: NextRequest) {
  ...
  try {
    if (type !== 'email' && type !== 'phone') {
      return NextResponse.json({ error: 'Dogrulama tipi gecersiz.' }, { status: 400 })
    }

    const isValidOtp = verifyEmailOtp(
      request.cookies.get(OTP_COOKIE_NAME)?.value,
      normalizedIdentifier.identifier,
      token
    )
    if (!isValidOtp) {
      return NextResponse.json({ error: 'Dogrulama kodu hatali veya suresi doldu.' }, { status: 401 })
    }
    ...
    if (isSignup) {
      ...
      response.cookies.set(OTP_COOKIE_NAME, '', { path: '/', maxAge: 0 })
      return response
    }
    ...
    return NextResponse.json({ error: error?.message || 'Dogrulama tamamlanamadi.' }, { status: 500 })
  }
}
