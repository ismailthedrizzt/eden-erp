...
const handler = createFastApiProxyHandler('/api/v1/companies')

export async function GET(request: NextRequest) {
  const query = parseListQuery(request.nextUrl.searchParams, { pageSize: 50, sort: 'phone' })
  const canonicalQuery = new URLSearchParams()
  canonicalQuery.set('page', String(query.page))
  canonicalQuery.set('pageSize', String(query.pageSize))
  canonicalQuery.set('sort', query.sort || 'phone')
  ...
  const response = await proxyToFastApi(request, '/api/v1/companies', {
    internal: true,
