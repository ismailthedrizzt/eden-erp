...
// TARGET_FASTAPI_ENDPOINT: /api/v1/after-sales/assets
...
const handler = createFastApiProxyHandler('/api/v1/after-sales/assets')

export async function GET(request: NextRequest) {
  const query = parseListQuery(request.nextUrl.searchParams, { pageSize: 50, sort: 'created_at', direction: 'desc' })
  const canonicalQuery = new URLSearchParams()
  canonicalQuery.set('page', String(query.page))
  canonicalQuery.set('pageSize', String(query.pageSize))
  canonicalQuery.set('sort', query.sort || 'created_at')
  canonicalQuery.set('direction', query.direction || 'desc')
  ...
  const response = await proxyToFastApi(request, '/api/v1/after-sales/assets', {
    internal: true,
